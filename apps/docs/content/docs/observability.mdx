...
`gateway: true` posts batches to `/v1/traces` with the publishable key. `webhookUrl` sends the same event payload to your endpoint with retry/backoff, queuing failed events in browser storage for the next successful delivery. `debug: true` also enables a console exporter.
