...
The **Site memory** page in the dashboard renders the exact snapshot each site
hands to agents (the same JSON delivered over WebMCP and the well-known
endpoint), so you can see what an agent sees. Alongside it, the snapshot is
scored **0–100** for how useful it is to an agent — weighing completeness,
clarity, and actionability — with concrete suggestions for what to add.

Scoring runs server-side in the dashboard via Vertex AI Gemini when configured,
and falls back to a deterministic completeness check (does the snapshot have a
site map, navigation, workflows, policies, UI guidance, and per-page context?)
so a score always renders. The numbers the score reasons over come from the
snapshot itself — nothing is invented.
